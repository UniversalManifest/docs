...
**Date:** 2026-05-02
**Status:** Formal work now extends through `WO-0256`. Universal Manifest v0.2 is published and the publication wave is closed through `WO-0231`, with the evidence pack at `docs/reports/2026-04-30-v0-2-publication-evidence-pack.md`. Phase 19 planning stubs remain defined at `WO-0243` through `WO-0251`, but the immediate post-publication adoption blocker is now the public front door: `WO-0252` is superseded as final homepage direction, `WO-0253` must produce a homepage-cluster design report, `WO-0254` must implement the approved design, `WO-0255` must verify production reader routes, and `WO-0256` must run real external-reader validation after those prerequisites and a real cohort/process exist. After revert commit `e493b3e` removed the unauthorized CI/workflow layer on 2026-04-28, Phase 9 remains manual-only.
...
- **Public landing/docs/discovery surfaces**
  ...
  - Stable latest published spec route: `site/src/pages/spec/latest.astro`
  - Durable v0.2 spec route: `site/src/pages/spec/v0.2.astro`
...
- **v0.2 published signature profile (artifacts exist)**
  - Signature profile: `spec/v0.2/SIGNATURE-PROFILE.md`
  - Published schema + context: `spec/v0.2/schema.json` + `spec/v0.2/schema.jsonld`
  - Published conformance: `spec/v0.2/CONFORMANCE.md`
...
- v0.1 signature is still a permissive placeholder; v0.2 now defines the published interop profile, while broader external adopter feedback remains post-publication work
- Phase 19 is defined as a post-publication planning queue, but its stubs (`WO-0243` through `WO-0251`) have not been promoted into full active WO files
- `WO-0253` through `WO-0255` must complete before reader validation resumes: design report, approved implementation, and production route verification
- `WO-0214` / `WO-0256` are blocked until a real external-reader cohort, consent/process, and transcript-capture plan are supplied
...
## Late-April queue reality (2026-04-30)
...
- Publication is executed. `WO-0220` cut and pushed `spec-v0.2`, and the GitHub release was published at `https://github.com/grigb/universal-manifest/releases/tag/spec-v0.2` on 2026-04-30.
- The publication surface wave is complete through `WO-0231`: `WO-0221` through `WO-0226` flipped the public spec, discovery, migration, conformance, governance, changelog, home-page announcement, and announcement-copy surfaces to the published v0.2 posture; `WO-0229` reconciled authority docs; `WO-0230` defined Phase 19; and `WO-0231` produced the publication closeout evidence pack. `WO-0214` is now blocked on real external-reader inputs and is represented by Phase 19 stub `WO-0243`.
...
## Early-May objective realignment (2026-05-02)

- `WO-0252` is retained only as historical evidence of the first homepage rewrite attempt and must not be extended as the final design direction.
- `WO-0253` is the immediate active work: produce a design report for the homepage cluster from the self-contained remote-agent brief at `/Users/grig/work/repo/universalmanifest/.dev/ai/designs/2026-05-01-homepage-cluster-redesign-agent-brief.md`.
- `WO-0254` is blocked on the accepted `WO-0253` design direction and covers implementation across `/`, `/use-cases/`, `/explorer/`, `/how-it-works/`, and `/standards-fit/`.
- `WO-0255` is blocked on implementation/deployment and covers production route verification for the public reader path.
- `WO-0256` is blocked on design, implementation, route verification, and real human reader inputs. Do not simulate external readers.
...
- For v0.2: verify signatures when present and required by the published profile
