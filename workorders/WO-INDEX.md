...
- WO-0255 (COMPLETED 2026-05-05): `docs/workorders/WO-0255-public-route-and-deployment-verification-v0-2-reader-path.md` — Cloudflare credentials were repaired, the deploy artifact now includes hidden `.well-known` files, final gated production deploy passed, and all required production reader/discovery routes returned `200`; closeout recorded in `docs/reports/2026-05-05-v0-2-reader-path-production-verification.md`
- WO-0256 (BLOCKED): `docs/workorders/WO-0256-external-reader-validation-execution.md` — run real external-reader validation after the user supplies a real 3-5 person cohort, consent/process expectations, and transcript/evidence capture method; design, implementation, and production-route verification prerequisites are complete
...
- WO-0258 (COMPLETED 2026-05-02): `docs/workorders/WO-0258-ci-and-staging-gate-remediation-after-homepage-redesign.md` — repaired CI/staging gate drift after the homepage redesign; `verify` and `CI/CD Pipeline` are green, and deploy now fails directly on Cloudflare `Invalid access token [code: 9109]`
